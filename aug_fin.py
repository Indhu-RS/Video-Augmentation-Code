...
def augment_videos(main_folder_path, output_folder_path, crop_size):
    ...
    augmentations = ['left_rotate', 'right_rotate']
    ...
    for filename in os.listdir(main_folder_path):
        if filename.endswith(".MOV") or filename.endswith(".mp4") or filename.endswith(".mov") or filename.endswith(".MP4"):
            ...
            # Apply random left rotate and right rotate augmentations on the cropped video
            for aug in augmentations:
                ...
                for frame in augmented_frames:
                    out.write(frame)
                out.release()
                print(f"{aug.replace('_', ' ').capitalize()} completed for {filename}")
...
def main():
    ...
    parser.add_argument('--crop-size', type=int, nargs=2, default=[720, 850], help='Crop size for center cropping in the format (height, width)')
